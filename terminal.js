...
function renderArchiveList() {
    ...
    if (keyword) filtered = filtered.filter(i =>
        (i.id + i.title + i.tags.join(' ') + i.summary + i.content).toLowerCase().includes(keyword)
    );
    const list = document.getElementById('archiveList');
    list.innerHTML = filtered.length ? '' : '<div class="archive-card" style="text-align:center;color:#ff3333;">[!] 无结果</div>';
...
function startTypewriter() {
    ...
    const timer = setInterval(() => {
        if (i < msg.length) el.textContent += msg.charAt(i++);
...
function initTerminal() {
    ...
    document.getElementById('resetDefaultBtn').addEventListener('click', () => {
        if (confirm('重置所有档案为默认？')) {
            archiveData = JSON.parse(JSON.stringify(DEFAULT_ARCHIVES));
            saveArchive();
            renderAdminList();
            renderArchiveList();
        }
    ...
    document.getElementById('insertImageBtn').addEventListener('click', () => {
        const url = prompt('图片链接：');
        if (url) document.getElementById('editContent').value += `<img src="${url}" style="max-width:200px;">`;
    });
